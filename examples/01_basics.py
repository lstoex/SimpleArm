...
from simplearm.jacobians import joint_jacobians, sphere_jacobians_from_joint_jacobians, com_jacobians_from_joint_jacobians
...
robot = RobotInfo.from_linklengths(linklengths) #This computes the robot metadata, including spheres and inertias, from the link lengths.
print(robot)
#%%
q = np.random.uniform(
...
print("Frames are shaped", frames.shape) #There are n_dof + 1 frames, each with a 3x3 homogeneous transform. The last frame is the end effector frame.
#%%
#Compute the joint jacobians for each link frame.
jacobians = joint_jacobians(frames)
print("Jacobians are shaped", jacobians.shape) #There are n_dof link frames, each with a 3 x n_dof spatial jacobian.
#%%
...
# %%
sphere_jacobians = sphere_jacobians_from_joint_jacobians(frames, jacobians, spheres_world)
print("Sphere Jacobians are shaped", sphere_jacobians.shape) #There are S spheres, each with a 3 x n_dof jacobian.
# %%
com_jacobians = com_jacobians_from_joint_jacobians(frames, jacobians, robot.linklengths)
print("COM Jacobians are shaped", com_jacobians.shape) #There are n_dof link COMs, each with a 3 x n_dof jacobian.
# %%
pairwise_dists, (pair_i, pair_j) = pairwise_sphere_dist(spheres_world, robot.ignore_pairs)
print("Pairwise distances are shaped", pairwise_dists.shape) #Of course we only compute pairs (i,j) where i < j, so there are (S-1)*S/2 - len(ignore_pairs) pairs.
...
q_traj = np.random.uniform(low=-np.pi, high=np.pi, size=(10, robot.n_dof)) #A random trajectory of 10 configurations.
...
sphere_jacobians_traj = sphere_jacobians_from_joint_jacobians(frames_traj, jacobians_traj, spheres_traj)
com_jacobians_traj = com_jacobians_from_joint_jacobians(frames_traj, jacobians_traj, robot.linklengths)
pairwise_dists_traj, (pair_i_traj, pair_j_traj) = pairwise_sphere_dist(spheres_traj, robot.ignore_pairs)
# %%
