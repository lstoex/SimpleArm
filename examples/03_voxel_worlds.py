...
g = SquareGrid.from_random_perlin(length=3., number_of_vox=64, res=4, layers=2) #resolution controls how "wild" the noise is, higher res means more variation in the noise. Layers control how many independent perlin noise layers are generated and combined.
...
fig = px.imshow(np.stack([g.data.T, sdf.data.T], axis=0), animation_frame=0, title="Perlin Noise (0) and Derived SDF (1)", origin="lower")
fig.show()
# %%
...
x, y = np.meshgrid(np.arange(x0, x1, sdf_grad_x.voxel_size), np.arange(y0, y1, sdf_grad_x.voxel_size), indexing='ij')
data_x = sdf_grad_x.data
data_y = sdf_grad_y.data #I know, this is cumbersome.... Care to improve it? Feel free :)
